...
On any nebula mutation, the initial implementation **re-projects all nebula entities** into `pulsar.state[path]`, replacing the entire projection. This is O(N) per mutation instead of O(1), where N is the total number of entities. Acceptable for early applications with small entity sets (dozens to low hundreds); does not scale to thousands.
...
- Respects set semantics (no re-projection on nebula no-ops such as duplicate `link`).
- Projected shape matches `nebula.get()` exactly (§3).
...
- **Camino 1:** Inverse index in the bridge (adapter-side, no nebula changes).
- **Camino 2:** Full-scan in `delete` (adapter-side, no nebula changes; recommended default).
- **Camino 3:** Opt-in observability API in nebula (requires nebula contract review).

Camino 2 is the recommended default for the first reactive implementation: it does not touch nebula, keeps the adapter self-contained, and accepts an O(N) cost only on `delete` (rare in target use cases like the diagram editor). Alternative caminos remain available if evidence suggests otherwise.
