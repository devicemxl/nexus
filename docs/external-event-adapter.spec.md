...
**Scope:** Bidirectional cross-tab synchronization of nebulaJS mutations via `BroadcastChannel`, with explicit anti-echo protection.


## 1. Purpose

Multiple browser tabs of the same application, running against the same nebula, currently do not see each other's mutations until one of them reloads and re-hydrates from persisted storage (see Persistence Adapter). This adapter closes that gap: when a nebula mutation happens in Tab A, it is broadcast to peer tabs; when a peer tab receives it, the same mutation is applied to its local nebula.
...
type MutationEvent = {
  type: 'nebula-mutation';   // Discriminator for coexistence with other channel messages.
  origin: string;              // UUID of the emitting tab. Enables anti-echo.
  op: MutationOp;              // Which nebula method was called.
...
- `context.nebula`: the nebula instance whose mutations will be broadcast and against which incoming mutations will be applied.
...
- `options.onRemoteError`: invoked when applying a remote mutation to local nebula throws (e.g., `link` referencing an entity that does not exist locally). Default logs to `console.warn`. The error does not propagate out of the message handler.
...
- `destroy()`: unwraps the nebula methods, closes the channel (if owned by the adapter), and marks the adapter inert. Idempotent.
...
On instantiation, the adapter wraps the seven nebula mutation methods (`put`, `upsert`, `update`, `delete`, `link`, `unlink`, `unlinkAll`) using the same pattern as the Bridge and Persistence adapters. Each wrapper calls through to the original, then:

1. Constructs a `MutationEvent` with `type: 'nebula-mutation'`, `origin: this.origin`, `op: <method name>`, and `args: [<the arguments the wrapper received>]`.
...
3. Uses the **original** nebula method (captured before wrapping) to apply the mutation. This is critical: applying via the wrapped method would re-broadcast the event, creating a loop. The original method mutates nebula but does not trigger the outbound wrapper.
...
1. Restore the seven original nebula methods.
...
Both wrap the same nebula methods. Order of instantiation matters:

- **Persistence outer, External inner (Persistence instantiated last):** local mutations first broadcast, then persist. Remote mutations arrive via the External wrapper (bypassed by the "original" trick), get applied to the raw nebula, and are not persisted by this tab — because they went through the original method that Persistence never saw. Consequence: remote mutations are visible in-memory but do not update this tab's `localStorage`. Since Persistence's key is per-tab-shared and any tab's mutation will persist from that tab's own outbound path, the network eventually reaches a consistent persisted state, but with a race.
...
```
nebula.put(...)  →  External wrapper (broadcast)
...
| **Set semantics respected** | `link`/`unlink`/`unlinkAll` that are nebula no-ops do not emit an event. |
...
| **Original API preserved on destroy** | After `destroy()`, `nebula.link(...)` etc. behave exactly as before instantiation. |
...
- **Does not synchronize Pulsar state.** Only nebula. If UI state (selection, viewport) needs to be shared across tabs, that is a separate concern.
...
- **Major:** Changes to the factory signature, changes to the wire event shape, changes to the anti-echo model, or changes to which nebula methods are wrapped.
