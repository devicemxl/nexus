...
**Scope:** Observes nebulaJS mutations and persists the entire graph to a storage backend, so that on the next application load a Hydration Adapter can restore state.


## 1. Purpose

Persistence is the write-side complement of Hydration. Hydration reads a snapshot at startup and populates nebula; Persistence observes nebula mutations and writes the graph back to storage. Together they close the loop `storage → nebula → mutations → storage`, letting an application recover its state across reloads without a backend.
...
type PersistenceOptions = {
  ...
  writeOnInit?: boolean;          // Default: false. If true, writes current nebula state immediately at instantiation.
...
- `context.nebula`: the nebula instance to observe.
...
- `options.mode`:
  - `'eager'`: every nebula mutation triggers an immediate serialize+write. Simpler but more expensive under bursty loads.
...
- `options.writeOnInit`: if `true`, serializes and writes the current state of nebula at adapter instantiation. Default `false` because the canonical flow (Hydration + Persistence) has the adapter mounted right after Hydration, and immediately re-persisting what was just read is redundant. Useful when Persistence is mounted on a pre-populated nebula that has no prior storage record.
...
- `destroy()`: unwraps the nebula methods, cancels any pending timer, and marks the adapter inert. Idempotent. **Does not flush by default** — the application decides whether pending state should be persisted before shutdown (typically via `flush()` before `destroy()`).
...
On instantiation, the adapter wraps the eight nebula mutation methods (`put`, `upsert`, `update`, `delete`, `link`, `unlink`, `unlinkAll`) using the same pattern as the nebula↔Pulsar Bridge (§3 of that mini-spec). Each wrapper calls through to the original and then schedules a write according to `mode`:
...
The adapter iterates `nebula.allIds()`, calls `nebula.get(id)` for each, and builds the canonical shape:
...
1. Restore the eight original nebula methods (mirror of what wrapper installation did).
...
// From here, all mutations to nebula are persisted with 300ms trailing debounce.
```

- **With nebula↔Pulsar Bridge:** Persistence and Bridge both wrap the same eight nebula methods. Order of instantiation determines wrapping order: the last one instantiated becomes the outer wrapper. Recommended order is **Bridge first, Persistence second** — mutations flow to Pulsar (fast, in-memory, drives UI) and then to storage (slow, best-effort). Reversing the order still works but delays UI updates behind the persistence timer in `'eager'` mode.

- **With multiple graphs:** an application with multiple independent nebula instances instantiates one Persistence Adapter per graph, each with its own `key`. The adapter does not multiplex.
...
| **Set semantics respected** | `link`/`unlink`/`unlinkAll` calls that are nebula no-ops do not schedule a write. |
...
- **Does not persist Pulsar state.** Only nebula. If UI state (selection, viewport, active view) needs to survive reloads, that is a separate concern — either serialize a `ui.*` slice manually, or wait for a future Pulsar Persistence Adapter.
