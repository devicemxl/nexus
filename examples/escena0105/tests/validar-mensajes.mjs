...
console.log('\n  Widget sobre DOM');
{
  ...
  // SUSTITUCIÓN DECLARADA EN LA ESCENA 1.5.
  //
  // Estas aserciones contaban reconstrucciones completas. Con reconciliación
  // con clave ya no hay reconstrucciones que contar, así que la métrica pasa a
  // ser la que sirve para los dos diseños y distingue lo que importa: cuántos
  // nodos se crean y cuántos se destruyen.
  asaWidget.reiniciarCuenta();
  const asa = provider.responder(conv);
  const trasAbrir = asaWidget.cuenta;
  eq('abrir el mensaje crea un nodo y no destruye ninguno',
    [trasAbrir.creados, trasAbrir.destruidos], [1, 0]);

  asaWidget.reiniciarCuenta();
  const tokens = p.agotar() - 1;          // la última pasada cierra, no emite
  const trasStream = asaWidget.cuenta;

  ok('el streaming emitió varios tokens', tokens >= 10, `tokens: ${tokens}`);
  eq('el streaming no crea ni destruye nodos',
    [trasStream.creados, trasStream.destruidos], [0, 0]);
  eq('y tampoco mueve ninguno', trasStream.movidos, 0);
  ...
  const trasInt = asaWidget.cuenta;
  eq('abrir e interrumpir crean un solo nodo y no destruyen ninguno',
    [trasInt.creados, trasInt.destruidos], [1, 0]);
