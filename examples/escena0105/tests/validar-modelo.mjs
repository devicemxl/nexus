...
console.log('\n  Modelo — derivación de la lista');
{
  ...
  // El empate sólo puede llegar desde almacenamiento: el reloj monótono lo
  // impide en ejecución. Se desempata en la MISMA dirección que el criterio
...
console.log('\n  Reloj monótono');
{
  ...
  ok('no se aleja del tiempo real mas de lo que dura la rafaga',
    Math.abs(marcas[marcas.length - 1] - Date.now()) < 1000);

  const { graphlet } = crearPrimitivas();
  const ids = [];
  for (let i = 0; i < 6; i++) ids.push(modelo.crearConversacion(graphlet, { titulo: `C${i}` }));
  const marcasCreadas = ids.map((id) => graphlet.get(id).properties.actualizadaEn);
  eq('seis creaciones seguidas no comparten marca', new Set(marcasCreadas).size, 6);

  const entities = {};
  for (const id of graphlet.allIds()) entities[id] = graphlet.get(id);
  eq('la creada en ultimo lugar encabeza la lista',
